...
export default function TakeTest(props) {
    ...
    const checkedCheck = () => {
        ...
        dummyTest.questions[currentQuestion].options.forEach((opt, index) => {
            ...
            if (opt == answerList[currentQuestion] || opt === answerList[currentQuestion]){
                console.log('the thing', 'opt: ', opt, 'ind: ', index, 'ans: ', answerList[currentQuestion]);
                if(index === 0)
                {
            ...
            {(() => {
                if(props.loggedIn){
                    if (props.isTeacher){
                        ...
                        return (
                        <div>
                            <h2>You are taking: {dummyTest.title} #{props.match.params.testid}</h2>
                            <h4>By {dummyTest.creator}</h4>
                            <h5>Question: {currentQuestion+1}/{dummyTest.questions.length}</h5>
                                ...
                                if (dummyTest.questions[currentQuestion].type === 'multiple-choice'){
                                    ...
                                    <div>
                                        ...
                                        {dummyTest.questions[currentQuestion].options.map((opt, index)=>{
                                            return <p><input type="radio" name='second' value={opt} onChange={handleChange} checked={isChecked[index]}/>{opt}</p>
                                        })}
                                        </form>
                                    </div>)
                                }
